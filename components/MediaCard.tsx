...
export default function MediaCard({ media, interests, members, activeUserId, hasComments, onToggleInterest }: Props) {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm overflow-hidden flex relative ${dimClass}`}>
      {/* Poster */}
      ...
      <Link href={`/media/${media.id}`} className="flex-1 p-3 min-w-0 block">
        <div className="flex items-start gap-2 mb-1">
          <h2 className="font-semibold text-gray-900 text-sm leading-tight flex-1 truncate">{media.title}</h2>
          <span className={`shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded ${media.type === 'movie' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
        ...
        {media.summary && (
          <p className="text-xs text-gray-600 line-clamp-2 mb-2">{media.summary}</p>
        )}
        ...
        <div className="flex items-center gap-0.5 flex-wrap">
          ...
          {hasComments && <span className="ml-1 text-gray-400 text-xs">💬</span>}
        </div>
      </Link>
    </div>
  )
